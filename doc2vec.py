...
def load_glove(filename):
    ...
    for line in f.readlines():
        ...
        dict[split_line[0].decode('latin-1')] = np.array(split_line[1:])
    f.close()
...
def words(text):
    ...
    mywords = [w for w in text.split(' ') if len(w) >= 3 and not w in ENGLISH_STOP_WORDS]
    return mywords
...
def load_articles(articles_dirname, gloves):
    ...
    myfilelist = filelist(articles_dirname) #get filelist
    for filename in myfilelist:
       if not 'COPYRIGHT' in filename: #delete the file "COPYRIGHT"
            new_filename = filename[len(articles_dirname)+1:]
            ...
            mylist.append((new_filename, title, article, wordvec_centroid))
...
def doc2vec(text, gloves):
    ...
    not in gloves.
    """
    accu_vec = np.zeros(shape=(len(gloves.values()[0]),1))
    count = 0
    for w in words(text):
        try:
            accu_vec = np.add(accu_vec, gloves[w])
            count += 1
        except:
            pass

    return accu_vec / count
...
def distances(article, articles):
    ...
    """
    return [(np.linalg.norm(article[3]-a[3]), a) for a in articles if article != a]
...
def recommended(article, articles, n):
    ...
    """
    return [d[1] for d in sorted(distances(article, articles))[:n]]
